...
Director Workspace (page title)
  +-- same top line: compact, collapsible Project/Course selector
  +-- Project status / 專案狀態摘要 (collapsed by default)
  |     +-- pipeline, stage/gate, authority health, PUP axes, cost
  |     +-- diagnostics
...
Project/Course choices stay compact until a user opens the selector; bounded loaded results must not be presented as the whole catalog, and pagination may only be exposed within that compact selection mechanism. Project Status retains its expanded state for a same-project stage change or refresh and resets when the authenticated project changes. URL query／fragment 保存 stage 與 ResourceRef selection；重新整理、browser back/forward 與貼上 deep link 都必須可還原。Lenses 到 B6 才加入，但 B1/B2 inspector components 必須能在不改變 projection contract 的前提下被 Lens 重用。
