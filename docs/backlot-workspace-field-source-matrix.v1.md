...
| Script revisions | manifest-declared unique owner checkpoint via `read_checkpoint` + `validate_artifact("script")` | Completed/gated revision may be canonical; awaiting is candidate. Working/failed are unavailable in B1B because v1 has no display-snapshot slot. Missing/invalid is unavailable; no timing continuity/ID uniqueness/duration-sum claims | revision set, resource/revision refs | B1B |
| Legacy script | contained loose script only under explicit legacy fallback | Still `display_only + unverified` in the matrix, but B1B deliberately does not implement this `b0.2_contained_reader_gap`; invalid authoritative evidence blocks fallback | projected revision | deferred B0.2/B1 |
